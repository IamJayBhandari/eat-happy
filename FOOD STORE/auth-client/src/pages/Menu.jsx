...
//                             <div className="absolute text-xl top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">{ curl.Elem?.price}</div>
//                         </button>
//                     </div>
//                 </div>
...
//                 </div>
//             </div>
//             <button className="bg-[#f54748] active:scale-90 transition duration-150 transform hover:shadow-xl shadow-md rounded-full px-8 py-2 tet-xl font-medium text-white" onClick={()=>addToCart(curElem)}>Add to cart</button>
...
//     { id: 1, name: 'Rice', value: 'Rice' },
//     { id: 2, name: 'Desert', value: 'Desert' },
//     { id: 3, name: 'Drinks', value: 'Drinks' },
//     { id: 4, name: 'Fruits', value: 'Fruits' },
//     { id: 5, name: 'Fast Food', value: 'Fast Food' },
//     { id: 6, name: 'Indian ', value: 'Indian ' },
//     { id: 7, name: 'Home Food', value: 'Home Food' },
//   ];
...
//   useEffect(() => {
//     getFoods();
//   }, [value]); // Fetch foods whenever the value changes
...
//                 key={btn.id}
//                 className={`text-xl px-4 py-3 text-center font-medium ${active === btn.id ? 'text-white bg-red-500 border-red-500 border-2 rounded-sm' : 'text-red-500 border-red-500 border-2'}`}
...
// <div className="grid py-6 gap-8 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1">
//   {food && food.map((curElem) => (
//     <div key={curElem._id} className="food-card bg-red-500/10 rounded-xl flex flex-col cursor-pointer items-center p-5">
//       <div className="relative mb-3">
//         <Link to={`/menu/${curElem._id}`}>
//           <img src={curElem.foodImage} alt={curElem.name} />
//         </Link>
//         {/* Add heart and price components here */}
//       </div>
//       <div className="flex gap-4 items-center">
//         <p className="text-xl text-center font-bold text-[#f54748]">
//           {curElem.name}
...
//         </p>
//         <div className="flex text-sm space-x-2 cursor-pointer">
//                     <span className="font-normal text-red">4.3</span>
//                     <FaStar size={16} className='text-[#fdc55e]'/>
//                    <span className="font-medium">({curElem?.reviews?.length})</span>
//                </div>
//         {/* Add star and review components here */}
//       </div>
//       <button
//         className="bg-[#f54748] active:scale-90 transition duration-150 transform hover:shadow-xl shadow-md rounded-full px-8 py-2 tet-xl font-medium text-white"
        ...
        <div className="p-5 mb-14">
          <div className="flex flex-wrap justify-center mb-8 gap-5">
            {category.map((btn) => (
              <button
                key={btn.id}
                className={`text-xl px-4 py-3 text-center font-medium ${active === btn.id ? 'text-white bg-red-500 border-red-500 border-2 rounded-sm' : (btn.value === 'Home Food' ? 'text-white bg-green-500 border-green-500 border-2 rounded-sm' : 'text-red-500 border-red-500 border-2')}`}
                onClick={() => handleBtn(btn)}
              >
                {btn.name}
              </button>
